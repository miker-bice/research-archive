...
$dbfield = array('ul.login_date','ul.logout_date','ul.ip_address','ul.device','CONCAT(u.firstname," ",u.lastname) as name'); // need iset based sa table columns
...
$default_query ="SELECT ".$field_query." FROM ".$table_name." as ul  LEFT JOIN (SELECT firstname,lastname,user_id FROM users) as u ON u.user_id = ul.user_id  ".$sql_conds;
$total_query = 0;
...
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where; 
$default_query ="SELECT ".$field_query." FROM ".$table_name." as ul  LEFT JOIN (SELECT firstname,lastname,user_id FROM users) as u ON u.user_id = ul.user_id  ".$sql_conds."   ORDER BY ".$orderby;
